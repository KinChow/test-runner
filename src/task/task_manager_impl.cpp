...
bool TaskManagerImpl::Remove(uint64_t task_id)
{
    ...
    if (task_it == tasks_.end()) {
        LOGW("Attempted to remove non-existent task ID: %zu", task_id);
        return false;
    }
    ...
    if (state != TaskState::Completed && state != TaskState::Failed) {
        LOGW("Attempted to remove active task ID: %zu (state: %d)", task_id, static_cast<int>(state));
    }
...
void TaskManagerImpl::CleanupExpiredTasks()
{
    ...
    for (auto task_id : expired_task_ids) {
        LOGW("Task %zu has expired, removing it", task_id);
        recycled_ids_.push(task_id);
        task_creation_times_.erase(task_id);
